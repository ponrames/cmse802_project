...
# Downsample the 'Load' column to hourly intervals
df_resampled = df_numeric.resample('H').mean()

# Perform time series split
train_size = len(df_resampled) - 144
train, test = df_resampled.iloc[:train_size], df_resampled.iloc[train_size:]

# Fit a SARIMA model to the training data with lower seasonal order
# (p, d, q) x (P, D, Q, s) parameters
model = SARIMAX(train['Load'], order=(1, 1, 1), seasonal_order=(1, 1, 1, 24))  # Example parameters for 24-hour seasonality
